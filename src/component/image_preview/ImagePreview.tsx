...
interface ImagePreviewProp {
  setCroppedCloudImg: ( imageURL: string) => void;
  defaultImg: string
}
const ImagePreview: React.FC<ImagePreviewProp> = ({ setCroppedCloudImg, defaultImg }) => {
  const [images, setImages] = useState<string | null>(defaultImg);//store the dataURL 
  ...
  const setCroppedImageFor = ( croppedImageUrl: string) => {
    ...
    setCroppedCloudImg(croppedImageUrl)
    onCancel()
  };
            ...
            <div className="carousel carousel-center mb-3 bg-gray-200 space-x-4 w-full p-2">
                ...
                <div  className="carousel-item border border-slate-400 h-48 w-48 relative">
                  ...
                  <button
                    ...
                    onClick={(e) => updateSelected(e, images)}
                  >
                    <CiCrop />
                  </button>
                </div>
